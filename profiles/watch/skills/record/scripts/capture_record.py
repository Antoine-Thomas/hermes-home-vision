import sys, os, pathlib, datetime, time, subprocess
...
import sys as _sys
try:
    dur = int(_sys.argv[1]) if len(_sys.argv)>1 else 30
except: dur=30
...
try:
    ...
    print(f"cv2 record err: {e}")

if not made:
    # fallback: rafale photos -> gif/mp4 via PIL
    try:
...
if made and dest.exists():
    ...
    print("Echec enregistrement: aucune image captee (webcam absente ?).")
    sys.exit(1)
