...
(
  for i in $(seq 1 380); do
    nvidia-smi --query-gpu=memory.used --format=csv,noheader,nounits >> vram_bf16.log 2>/dev/null
    sleep 30
  done
) &
MON=$!
...
timeout 10800 "$V/venv/Scripts/python.exe" -m accelerate.commands.launch \
  ...
  --output_dir="$V/lora_essai" --mixed_precision=bf16 \
  --checkpoints_total_limit=4 --report_to=tensorboard \
  > test_bf16.log 2>&1
echo "code : $?" >> test_bf16.log
kill $MON 2>/dev/null
