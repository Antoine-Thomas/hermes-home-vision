...
| Point | Nature | État |
|---|---|---|
| `docs\snapshot\env.pre_update.redacted` | **fuite réelle trouvée le 17/09** : la ligne 338 portait un **jeton Telegram du bureau en clair** (sha256[:16] `7da363461151846e`) dans un fichier dont le nom annonce « redacted ». `getMe` → **200 : le jeton était vivant**. Introduit par le commit `76975ae`, donc le blob **reste dans l'historique** | **valeur neutralisée** (commit `c2c81a1`, remplacée par `<TELEGRAM_BOT_TOKEN>`), suivi vérifié — mais **révoquer le jeton reste obligatoire avant tout push** : retirer une ligne ne retire pas le blob. Leçon : un fichier « .redacted » n'est pas redacté par sa nature, seulement par un contrôle de contenu |
...
## 8. Dépôts git — règle et état (fusion du 17/09/2026)
...
### État au 17/09/2026 (soir) — les deux dépôts n'en font plus qu'un

- **Fusion** : l'ancien dépôt de documentation a été fusionné dans le runtime (`docs/`), historique
  préservé : 92 commits + 15 côté runtime + 1 commit de fusion = **109**, puis les commits de
  documentation → le dépôt unique `hermes-home-vision` compte **110 commits**. Ancien dépôt local
  conservé hors circuit : `Desktop\hermes_install.archive`.
- **Purge d'historique : faite** (elle était reportée depuis le 17/09 au matin). Six chemins porteurs
  de secrets (`snapshot/env.pre_update.redacted`, `snapshot/.env`, `snapshot/state.db`,
  `backups/veille/.env.avant_telegram.20260917_134303`, `.env.avant_3d.20260917_125621`,
  `.env.final.MASQUE`) ont été retirés par `git filter-repo` : `.git` est passé de **80 Mo à 702 Ko**,
  et le re-scan de tous les blobs (`docs\scripts\scan_secrets_history.py`) donne **0 valeur réelle**
