...
def controle_qualite(video):
    """Appelle controle_qualite.py sur le fichier rendu : rend (verdict, chemin du rapport).

    Le controle se declenche apres le rendu en mode run. Pour le desactiver ponctuellement :
    V4_SANS_QUALITE=1. La source 4K (V4_SRC4K) est transmise si elle est definie, sinon les
    deux controles sur les levres sont simplement marques non comparables.
    """
    script = os.path.join(HERE, "controle_qualite.py")
    if not os.path.exists(script):
        print("controle qualite absent (%s) : ignore" % script, flush=True)
        return "ABSENT", ""
    if os.environ.get("V4_SANS_QUALITE") == "1":
        return "DESACTIVE", ""
    cmd = [sys.executable, script, video]
    src = os.environ.get("V4_SRC4K")
    ...
    for ligne in sortie.splitlines():
        if ligne.startswith("rapport ecrit :"):
            rapport = ligne.split(":", 1)[1].strip()
    return {0: "OK", 1: "ATTENTION", 2: "ALERTE"}.get(r.returncode, "INCONNU"), rapport
...
def main():
    ...
    # --- controle qualite automatique (Phase 1 : seuils et verdicts) ------------------
    # La video n'est jamais supprimee, meme en ALERTE : le rapport est ecrit dans le meme
    # dossier, et le pipeline sort en code 2 pour que l'appelant puisse s'arreter.
    verdict, rapport = controle_qualite(out)
    print("qualite : %s%s" % (verdict, " | %s" % rapport if rapport else ""), flush=True)
    if verdict == "ALERTE":
        print("ALERTE qualite : video conservee, consulter le rapport avant tout usage.",
              flush=True)
        sys.exit(2)
