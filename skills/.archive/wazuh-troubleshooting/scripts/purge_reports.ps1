...
# Deploy to: C:\ProgramData\Hermes\purge_reports.ps1
...
# SAFETY CHECK: Read CSV and verify no critical actions pending
$safeToPurge = $true
    ...
    foreach ($row in $csv) {
        ...
        foreach ($act in $actions) {
            ...
            if ($act -in @('isolate-host','forensics-image','revoke-keys','rotate-passwords')) {
                Write-Log "BLOCKED: action '$act' found for host $($row.host)"
                $safeToPurge = $false
            }
        }
        if ($row.alerts_wazuh_recentes -and $row.alerts_wazuh_recentes -notmatch '^0') {
            Write-Log "BLOCKED: non-zero alerts for host $($row.host)"
            $safeToPurge = $false
        }
    }
}
...
# Find archives older than RetentionDays
$cutoff = (Get-Date).AddDays(-$RetentionDays)
$archivesDir = "$BasePath\archives"
$quarantineDir = "$BasePath\quarantine"

if (Test-Path $archivesDir) {
    $oldArchives = Get-ChildItem $archivesDir -File | Where-Object { $_.LastWriteTime -lt $cutoff }
    ...
    Write-Log "SUMMARY: $($oldArchives.Count) to quarantine, $($staleQuarantine.Count) to delete"
}
