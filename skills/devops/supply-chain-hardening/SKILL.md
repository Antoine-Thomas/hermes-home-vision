...
## After audit — remediation triage

When `security-audit` reports vulns: fix `CRITIQUE` first (exposed SQL dumps, wp-config backups in webroot). For `deps`: run `npm audit fix` without `--force` first and verify build — `--force` bumps majors (e.g. @wordpress/scripts 35) and breaks. `httpx2`/`httpcore2` CVEs from `mcp` SDK fork are local-only and not fixable until `mcp` bumps its fork; do not chase with `pip install --upgrade httpx`. Always re-run audit to confirm `CRITIQUE 0` after file hygiene.
...
- **Fabricated test fixtures are not test data.** Before building anything on a
  ...
  reserved domains (`example.com`, `example.org`, `test`, `invalid`)
  cannot receive mail and generally mark a fixture you or someone else generated.
- **`npm audit fix` fails with `EALLOWGIT`** ("Fetching packages of type git have
  been disabled") when a dependency is pinned to a git source (`github:user/repo`).
  npm ≥11 defaults `allow-git=false`, so the audit fix cannot re-resolve the tree.
  Fix: replace the git dep with a registry version (`npm view <pkg> version` →
  `^x.y.z`), then re-run `npm audit fix` (no `--force`). Do NOT `npm audit fix --force`
  just to clear the error — it bumps majors (e.g. @wordpress/scripts 19→35) and breaks.
