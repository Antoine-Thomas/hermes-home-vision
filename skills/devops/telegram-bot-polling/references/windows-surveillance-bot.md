...
- `audio_guardian/guardian.py` — autonomous YAMNet audio detection (Door/Glass/Knock/...);
  triggers a video capture + `sendVideo` on detection. One-way sender: it never calls
  `getUpdates`, so it coexists with `surveillance.ps1` on the same token without conflict.
...
## A second Hermes gateway profile hijacks the bot token

A Hermes gateway answers EVERYTHING like an AI assistant. If a second Hermes profile
(e.g. `--profile watch`) is spun up and its gateway polls the surveillance bot's own
token (`token.sec`), the bot stops being a surveillance bot and becomes a "second chat":
the gateway consumes every `/watch` `/stopcam` `/video` AND replies conversationally,
while `surveillance.ps1` is starved. Symptoms are "the bot became a second chat" AND/OR
"the bot replies 'Unrecognized slash command /video' to its own commands" (the gateway
has no handler for them, and the real bot is down or starved) — not "the bot stopped
responding".

Diagnose:
1. `getMe` each candidate token — `first_name` identifies the bot (`"Bot video"` = the
   surveillance bot, `"Hermes"` = the assistant).
2. `getMe` is the DEFINITIVE check — both tokens resolving to the same `@username`/id
   means they are the same bot. A hash comparison is only valid on the RAW token value:
   hash `token.sec` content vs the `TELEGRAM_BOT_TOKEN` value stripped of its `KEY=`
   prefix and trailing newline, otherwise the same token hashes differently and you
   wrongly conclude "different tokens".
3. Find the rogue gateway: `Get-CimInstance Win32_Process -Filter "Name='python.exe'" |
   Where { $_.CommandLine -like '*gateway*' }` — a `--profile <name>` in argv names it.

Fix (make the dedicated bot the sole poller):
1. `hermes --profile <name> gateway stop`
2. `Disable-ScheduledTask -TaskName 'Hermes_Gateway_<name>'`  (guards the ONLOGON relaunch)
3. Start `surveillance.ps1` (hidden pwsh); confirm a probe `getUpdates` returns 409.
...
- **Omit `-video_size`/`-framerate` on the dshow INPUT.** Passing `-video_size 640x480
  -framerate 15` to a dshow webcam commonly fails with *"Could not set video options /
  I/O error"*. Let the device negotiate its own mode; the combined `video=X:audio=Y`
  input syntax works without size hints. Enforce size/fps only on the OUTPUT side if
  needed.
...
- **`Get-Process` shows a mic-listening Python process as "Not Responding"** — a
  sounddevice/TensorFlow read loop blocks without pumping Windows messages, so
  `guardian.py` (YAMNet) appears "Not Responding" while healthy. Verify liveness via
  its log file or CPU delta, not by killing it.
