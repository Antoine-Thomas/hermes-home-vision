...
la derniere revision : c'est lui qui dit si le declencheur « 10 nouveaux skills » ou « un mois »
est atteint.
"""
import argparse
import datetime
...
if __name__ == "__main__":
    ...
    if a.etat:
        ...
        print("jours depuis      : %s (declencheur a 30)" % (jours if jours is not None else "n/a"))
        declenche = nouveaux >= 10 or jours is None or jours >= 30
    ...
    lignes += ["", "## 4. Etat du declencheur", "",
               ...
               "| Jours depuis la revision | %s | 30 |" % (jours if jours is not None else "n/a"), ""]
