...
- **Never put a `/` in a document `path` or title, nor in a notebook name**: SiYuan maps both to the filesystem, so `path: "/Rapport - 21/09/2026"` silently creates `Rapport - 21` > `09` > a doc titled `2026`, and `createNotebook` strips the slash (title displayed wrong). Use dashes (`/Diagnostic Groq TPM 2026-09-21`) and put the pretty date inside the content. Cleanup of such a stray tree: `removeDocByID` deepest-first.
- **`updateBlock` on a list item ALSO replaces its children, and the SQL `content` column carries the whole subtree.** A `LIKE` on a child's text therefore matches its parent (and every ancestor), so an edit driven by that match wipes the sub-bullets — measured on a real edit: a 1 094-char section collapsed to a single line. `has_children` comes back `null` through `/api/query/sql`, so it cannot be used as a guard either. Safe recipe for editing an existing document through the API: (1) export the whole markdown with `/api/export/exportMdContent` and note its length; (2) pick the target block by a SQL match on the START of its content and restrict to `type='p'`/`h`/`c` (never `l` or `i`); (3) mutate that single block; (4) re-export and check the length plus the presence of EVERY heading you saw in step 1. To rewrite a whole section, `deleteBlock` the blocks then `appendBlock` the corrected markdown — never rewrite a section by updating one of its bullets in place.
- **Do not send your own YAML frontmatter in `markdown`**: SiYuan generates its own, so a leading `---\ndate: …\n---` block is rendered as body text (stray rule + literal lines). Pass metadata via `setBlockAttrs` (`custom-date`, `custom-tags`) instead.
