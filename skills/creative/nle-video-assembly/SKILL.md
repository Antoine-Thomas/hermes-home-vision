...
- **Framerate preservation rule**: Each segment generated via FFmpeg MUST preserve the source framerate until the final output stage — set `-r` only on the output, never as an input option.
- **Alternating A/B montage**: when the user wants `A 5 s, B 5 s, A 5 s …` from two long sources, use the ffmpeg `concat` demuxer with `inpoint`/`outpoint` in a single pass — do not generate 500+ intermediate files and do not call Premiere `insertClip` 550 times when Premiere is closed and ffmpeg is available. See `references/alternating-montage.md` for the exact `inpoint` math.
- **Mixed resolution / framerate sources**: normalize with `-vf "scale=1920:1080:force_original_aspect_ratio=decrease,pad=1920:1080:(ow-iw)/2:(oh-ih)/2:color=black,setsar=1,fps=30"` and `-r 30` on output. Expect `Reconfiguring filter graph` and `Non-monotonic DTS / Queue input is backward` at every A↔B junction — they are harmless and auto-corrected by ffmpeg. Do not abort on them.
- **Premiere MCP CEP (premiere-pro-mcp ≥1.15)**: there is no `npm run install-extension`. The real installer is `node dist/index.js --install-cep` (and `--diagnose-cep`, `--doctor`). Destination is `%APPDATA%\Adobe\CEP\extensions\MCPBridgeCEP` (not `com.mcp.premiere.bridge`). Enable `PlayerDebugMode=1` for every `HKCU\Software\Adobe\CSXS.*` (9-14), then restart Premiere and open `Window > Extensions > MCP for Adobe Premiere Pro`. See `references/premiere-mcp-cep.md`.
 include
