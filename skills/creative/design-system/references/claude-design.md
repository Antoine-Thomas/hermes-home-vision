<!-- Source: creative/claude-design/SKILL.md — migrated to umbrella design-system on 2026-09-10 -->
...
## Quick Sketch Mode (absorbed from `sketch` skill)

When the user wants to see design directions before committing — throwaway variants for comparison, not polished artifacts.
...
Name by stance, not number: `calm-editorial/`, `utilitarian-dense/`, `playful-split/`. Pick one axis (density, emphasis, aesthetic, layout) and pull apart from it. Two variants that differ only in accent color are wasted effort.
...
More than that is over-engineering; less than that is a screenshot.

### Frontier mode

If sketches exist and the user asks "what should I sketch next?", evaluate:
- Consistency gaps (independent choices not composed together)
- Unsketched screens (referenced but never explored)
- State coverage (empty/loading/error/1000-items)
- Responsive gaps (mobile/ultrawide)
- Interaction patterns (transitions, drag, scroll behavior)

### Output structure

```
sketches/001-stance-name/index.html + README.md
```

Each variant is a single self-contained HTML file with inline CSS, system fonts, and at least one interactive state transition.
