...
## Hermes — accès à Jev (adaptation locale, ajoutée en fin de skill)

Cette section est propre à cette installation Hermes : elle n'existe pas dans le
skill amont. Le reste du fichier est le `SKILL.md` officiel verbatim (v0.5.7,
commit `65a39f39`).

- Route déjà câblée : **OpenRouter**, `POST https://openrouter.ai/api/v1/systemone`,
  modèle `typesafe/jev-1.13` (réponse : `typesafe/jev-1.13-20260917`), clé
  `OPENROUTER_API_KEY` de `%LOCALAPPDATA%\hermes\.env`.
...
- Installation / mise à jour : contrairement à l'amont, **ne pas** lancer
  `claude plugin marketplace add` ni `npx skills add`; recopier le dossier
  `skills/typesafe-ai` amont dans `%LOCALAPPDATA%\hermes\skills\typesafe-ai\`
  puis ré-appendre cette section.
