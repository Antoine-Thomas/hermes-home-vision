...
- **Vérifier la chaîne complète, jamais le maillon qu'on vient de relancer.** Un `200` en direct sur
  ...
  qui tranche.
- **Un relais qui ne sait servir que du JSON complet doit FORCER `stream: false` vers l'amont.** Le
  client et le routeur streament par défaut : le drapeau est transmis tel quel au service amont, qui
  répond en **SSE** ; le relais tente alors de parser ce flux comme du JSON et répond
  `502 {"error": "Expecting value: line 1 column 1 (char 0)"}` (le message vient du relais, pas de
  l'amont). Piège de diagnostic : les essais directs en `stream: false` passent **tous**, donc le
  relais paraît sain alors que le chemin réel échoue à chaque appel. Reproduire avec `stream: true`
  avant d'accuser quoi que ce soit, puis poser `body["stream"] = False` (+ retirer
  `stream_options`) dans **chaque** méthode de transfert — le routeur refabrique le flux SSE côté
  client, donc rien n'est perdu.
