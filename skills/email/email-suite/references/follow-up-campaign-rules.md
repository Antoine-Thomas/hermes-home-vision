...
**Name-source priority matters.** When enriching `sent[]` names, prefer the
campaign CSV `prenom` that the previous email actually used over
`contacts_master.json['first']`. Observed: `contacts_master['first']` carried
garbage the CSV had correctly left empty (`hubbubhum@gmail.com` → `"d^_°b"`,
`florian.tirot@gmail.com` → `"Video"`), so prioritizing it turned a clean
`Bonjour,` into `Hello Video,`. Rule: CSV `prenom` first, `contacts_master['first']`
only as a fallback, and a name with no `@`/digits can still be garbage — when in
doubt (single word that looks like an email local part or a surname), leave the
name empty rather than personalize wrongly.
