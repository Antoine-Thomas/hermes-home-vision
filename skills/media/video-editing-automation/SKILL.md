...
- Create a fixed-interval alternating montage (e.g. 5 s A/B loop from two long sources) via FFmpeg concat demuxer — see `references/alternating-concat.md`
- Create an alternating montage WITH per-junction transitions (xfade wipe/fade/dissolve) — see `references/xfade-block-strategy.md` (shallow 30–60-segment blocks, never one giant chain)
...
- Render each segment to a temp file with `scale+pad+fps=60+format=yuv420p`, then xfade-chain; per-segment temp files beat one giant filter_complex. Keep xfade chains shallow (30–60 segments) — chain depth is the dominant speed knob, a 300-deep chain renders ~20x slower than a 60-deep one (`references/xfade-block-strategy.md`).
